...
class SettingsApi {
	 ...
	 * Unfortunately this is needed, because I do not know if this API
	...
	public function get() {
		if(!empty($this->args['get'])) {
			$item_array = \call_user_func_array(array($this, 'get' . EveOnlineDscanTool\Helper\StringHelper::camelCase($this->args['get']), true), array($this->args));
...
//					$complete_url = \wp_nonce_url(\admin_url('options-general.php?page=' . $page . '&callback=rsa_delete_settings'));
	...
	public function enqueueStyles() {
		if($this->isSettingsPage() === true) {
			...
			\wp_enqueue_style(
				'font-awesome',
				(\WP_DEBUG === true) ? EveOnlineDscanTool\Helper\PluginHelper::getPluginUri() . 'font-awesome/css/font-awesome.css' : EveOnlineDscanTool\Helper\PluginHelper::getPluginUri() . 'font-aweseome/css/font-awesome.min.css'
			);
			\wp_enqueue_style(
				'settings-api',
